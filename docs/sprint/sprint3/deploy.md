...
| 작업 | 내용 | 상태 |
|---|---|---|
| 대시보드 KST 버그 수정 | 서버 UTC 기준 날짜 계산 → KST(UTC+9) 기준으로 수정 | ✅ |
| 대시보드 날짜 선택 기능 | `‹/›` 버튼 + date input, `?date=` 쿼리 파라미터 지원 | ✅ |
| 시드 데이터 보완 | 3월 방문 데이터 추가 (하루 2~3건, 총 46건), DB 재시드 완료 | ✅ |
| 문서 정리 | deploy.md 브랜치 정보 오류 수정, 경로 수정 | ✅ |
| CLAUDE.md 규칙 추가 | 날짜/시간 처리 KST 기준 명문화 | ✅ |
