...
| 엔드포인트 | 방법 | 상태 | 응답 예시 |
|---|---|---|---|
| `/api/plans` | POST | ✅ 200 | `{ success: true, data: { id: "PL_...", steps: [...] } }` |
| `/api/plans/:id` | GET | ✅ 200 | `{ success: true, data: { canDispense: true, ... } }` |
| `/api/visits` | POST | ✅ 201 | `{ success: true, data: { id: "V_..." } }` |
| `/api/visits/:id/dispense` | POST | ✅ 200 | `{ success: true, data: { dispensedAt: "..." } }` |
| `/api/visits/:id/dispense` (2차, 이전 미완료) | POST | ✅ 400 | `{ success: false, code: "PREVIOUS_VISIT_REQUIRED" }` |

### 테스트 시나리오 결과

**TC-01: 환자 신규 등록 및 복약 관리 계획 수립**
- ✅ 환자 드롭다운에서 김영희(P001) 선택
- ✅ 처방 기간 90일, 방문 횟수 3회 입력
- ✅ 자동 계산: 조제 단위 30일, 방문 일정 3단계 생성
- ✅ POST /api/plans → 201 응답, plansStore에 저장

**TC-02: 1차 방문 - 정상 조제**
- ✅ 1단계 canDispense=true 확인
- ✅ 방문 기록 생성 (POST /api/visits → 201)
- ✅ 조제 처리 (POST /api/visits/:id/dispense → 200)
- ✅ 계획 단계 status: 'completed'로 업데이트

**TC-03: 정기 방문 - 복약 상태 기록**
- ✅ 복약 순응도/이상반응/보관상태/약사메모 저장
- ✅ 환자 상세 타임라인에서 이력 조회 확인

### 단위 테스트 결과 (Vitest)

```
✓ planCalculator: 5 tests passed
✓ canDispense: 4 tests passed
총 9개 테스트 통과
```
