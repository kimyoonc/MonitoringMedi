...
| M10: UX 개선 2차 | 추가 작업 | 방문 기록 교환 신청 복원, 보관 상태 연동 교환 자동 펼침, 환자 처방일수 표시 | ✅ 완료 |
| M11: 대시보드 개선 + 데이터 보완 | 추가 작업 | 대시보드 KST 날짜 기준 수정, 날짜 선택 기능, 3월 시드 데이터 보완 | ✅ 완료 |
| M12: 대시보드 차트 + UX 개선 | 추가 작업 | 주간 방문 추이 차트, 질환별 환자 수 차트, 환자 검색, 이상반응 붉은색, 환자 선택 필터 | ✅ 완료 |
| M13: UX 개선 4차 + 성능 개선 | 추가 작업 | 환자 관리 서브 메뉴, 첫 화면 소개 섹션, 대시보드 타이틀, 로딩 최적화 | ✅ 완료 |
...
| 작업 | 상태 |
|---|---|
| 방문 기록 작성 페이지(VisitRecordPage)에 교환 신청 폼 복원 — ExchangePage는 이력 조회 전용 유지 | ✅ |
| 의약품 보관 상태 셀렉트를 조제 의약품 확인 카드 아래로 이동, 교환 신청 체크박스 대체 | ✅ |
| 보관 상태 불량/훼손 선택 시 교환 신청 폼 자동 펼침, 양호 복귀 시 자동 닫힘 | ✅ |
| 환자 상세 기본 정보 카드에 처방일수 및 조제 완료 일수 표시 (예: 60일 / 180일) | ✅ |
| 테스트 환자 10명 추가 (P004~P013) | ✅ |

#### M11: 대시보드 개선 + 데이터 보완

| 작업 | 상태 |
|---|---|
| 대시보드 날짜 계산 KST(UTC+9) 기준으로 수정 (서버 UTC 기준 버그 수정) | ✅ |
| 대시보드 날짜 선택 기능 추가 — `‹` / `›` 버튼 + date input, `?date=` 쿼리 파라미터 지원 | ✅ |
| 대시보드 날짜 변경 시 환자 미표시 버그 수정 (early return 제거, UTC 기반 날짜 계산) | ✅ |
| 대시보드 날짜 조회 status 필터 제거 — 과거/오늘/미래 모두 표시, 미래 날짜 허용 | ✅ |
| 3월 방문 테스트 시드 데이터 보완 (하루 2~3건, 총 46건) | ✅ |
| CLAUDE.md 날짜/시간 처리 규칙 문서화 (KST 기준 일관 적용) | ✅ |

#### M12: 대시보드 차트 + UX 개선 3차

| 작업 | 상태 |
|---|---|
| 대시보드 주간 방문 추이 가로 막대 차트 추가 (선택 날짜 기준 최근 7일, 오늘 파란색 강조) | ✅ |
| 대시보드 질환별 환자 수 가로 막대 차트 추가 (전체 환자 conditions 집계, 상위 8개) | ✅ |
| PC에서 두 차트 2컬럼 나란히 배치, 높이 동기화 (align-items: stretch) | ✅ |
| 환자 관리 페이지 이름/질환 검색 기능 추가 | ✅ |
| 대시보드 이상반응 요약 카드 항상 붉은색으로 표시 | ✅ |
| 환자 역할 선택 화면 이나영 단일 표시 필터 | ✅ |
| 대시보드 빈 목록 섹션 어두운 배경색 제거 (EmptyState를 .list 래퍼 밖으로 분리) | ✅ |

#### M13: UX 개선 4차 + 성능 개선

| 작업 | 상태 |
|---|---|
| SideNav 환자 관리 서브 메뉴 추가 — 오늘 방문 예정(`/patients/scheduled`), 조제 대기(`/patients/pending`) | ✅ |
| 환자 상세 진입 시 서브 메뉴 자동 펼침 (pathname startsWith 감지) | ✅ |
| 첫 화면(RoleSelectPage)에 분할조제 소개 섹션 추가 (알약 비주얼, 3단계 카드, 진행 현황 바, 통계) | ✅ |
| 첫 화면 환자 카드 외 아무 곳 클릭 시 약사 대시보드로 이동 | ✅ |
| 대시보드 로딩 최적화 — 첫 화면 진입 시 백엔드 워밍업 prefetch, localStorage 캐시로 재방문 즉시 표시 | ✅ |
| 대시보드 상단 그라디언트 타이틀 추가 ("하나의 처방전을 나눠서 조제합니다.") | ✅ |
| E2E 테스트 자동 동기화 (UI 변경 시 smoke.spec.ts heading 셀렉터 함께 수정) | ✅ |
| CLAUDE.md 빌드 확인 / md 파일 배포 생략 / E2E 동기화 규칙 추가 | ✅ |
...
| UX 2차 | 방문 기록 교환 신청 복원, 보관 상태→교환 연동, 환자 처방/조제 일수 표시 | - | 추가 작업 | ✅ 완료 |
| 대시보드 개선 | KST 날짜 기준 버그 수정, 날짜 선택 기능, 시드 데이터 보완 | - | 추가 작업 | ✅ 완료 |
| 대시보드 차트 | 주간 방문 추이 막대 차트, 질환별 환자 수 가로 막대 차트 | - | 추가 작업 | ✅ 완료 |
| UX 3차 | 환자 관리 검색, 이상반응 카드 붉은색, 환자 선택 화면 이나영 필터, 빈 목록 배경색 제거 | - | 추가 작업 | ✅ 완료 |
| UX 4차 | 환자 관리 서브 메뉴, 첫 화면 소개 섹션, 대시보드 타이틀 | - | 추가 작업 | ✅ 완료 |
| 성능 | 대시보드 prefetch 워밍업 + localStorage 캐시 | - | 추가 작업 | ✅ 완료 |
