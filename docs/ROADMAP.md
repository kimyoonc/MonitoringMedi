...
| 작업 | 상태 |
|---|---|
| 대시보드 주간 방문 추이 SVG 막대 차트 추가 (선택 날짜 기준 최근 7일, 오늘 파란색 강조) | ✅ |
| 대시보드 질환별 환자 수 가로 막대 차트 추가 (전체 환자 conditions 집계, 상위 8개) | ✅ |
| PC에서 두 차트 2컬럼 나란히 배치, 높이 동기화 | ✅ |
| 주간 차트 방문 수 항상 표시 (0 포함), 질환 차트 높이 기준으로 세로 크기 맞춤 | ✅ |
...
| UX 3차 | 환자 관리 검색, 이상반응 카드 붉은색, 환자 선택 화면 이나영 필터 | - | 추가 작업 | ✅ 완료 |
