...
| M9: UX 개선 | 추가 작업 | 계획 수립 예시 카드, 교환 관리 환자 흐름 편입, 순응도 차트 개선 | ✅ 완료 |
...
| F-007 | 조제 대기 섹션, 환자 카드 링크, PC 3컬럼 레이아웃 | ✅ |
...
| 작업 | 상태 |
|---|---|
| 복약 관리 계획 수립 페이지 상단 사용 예시 카드 추가 (처방기간 → 방문예정횟수 → 자동계산 흐름 시각화) | ✅ |
| "방문 횟수" 문구 → "방문 예정 횟수"로 통일 | ✅ |
| 교환 관리를 환자 관리 흐름 내로 편입 (네비 독립 메뉴 제거, 독립 라우트 `/pharmacist/exchanges` 제거) | ✅ |
| 복약 순응도 차트 크기 축소 (CHART_H 80→52, BAR_W 28→20) 및 설명 문구 추가 | ✅ |
...
| UX | 계획 수립 예시 카드, 방문 예정 횟수 문구, 교환 관리 환자 흐름 편입, 순응도 차트 개선 | - | 추가 작업 | ✅ 완료 |
